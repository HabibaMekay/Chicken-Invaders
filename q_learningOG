...
def Q_learning(game_env):
    ...
    for episode in range(100):
        ...
        while not is_terminal_state(current_node) and steps < 1000:
            ...
            Q_table[state.position][actions[action_index]] += 0.9 * (reward + 0.1 * max(Q_table[next_state].values()) - Q_table[state.position][actions[action_index]])
...
def simulate_game(game_env, Q_table, algorithm_name):
    ...
    while game_env.chickens:
        ...
        algorithm_text = font.render(f"Algorithm: {algorithm_name}", True, WHITE)
        game_env.screen.blit(algorithm_text, (10, 10))

        current_state = game_env.ship_x
        if current_state in Q_table:
            best_action = max(Q_table[current_state], key=Q_table[current_state].get)
        else:
            best_action = "left"

        if best_action == "right" and game_env.ship_x < game_env.SCREEN_WIDTH - 60:
            game_env.ship_x += 5
            actions_taken.append("move_right")
        elif best_action == "left" and game_env.ship_x > 0:
            game_env.ship_x -= 5
            actions_taken.append("move_left")
        elif best_action == "shoot":
            if game_env.active_bullet is None:
                game_env.active_bullet = pygame.Rect(game_env.ship_x + 30, game_env.ship_y, 5, 10)
...
def main():
    ...
    actions = simulate_game(game_env, Q_table, "Q-Learning")
